...
// --- rate-limit (429) auto-retry ---------------------------------------------

function rateLimitThenSucceedModel(
...
test('handleChat retries a rate-limited turn with backoff, then succeeds', async t => {
	const counter = {n: 0};
	const result = await handleChat({
		model: rateLimitThenSucceedModel(2, counter),
		currentModel: 'test-model',
		providerConfig: stallProviderConfig,
		messages: [{role: 'user', content: 'test'}],
		tools: {},
	...
	t.is(counter.n, 2, 'rate-limited once, retried once');
	t.is(result.choices[0]?.message.content, 'recovered');
});

test('handleChat surfaces the error after exhausting rate-limit retries', async t => {
	const counter = {n: 0};
	// Never succeeds → every attempt is rate-limited.
	await t.throwsAsync(
		handleChat({
			model: rateLimitThenSucceedModel(99, counter),
			currentModel: 'test-model',
			providerConfig: stallProviderConfig,
			messages: [{role: 'user', content: 'test'}],
			tools: {},
	...
	// 1 initial attempt + MAX_RATE_LIMIT_RETRIES (3) retries = 4 doStream calls.
	t.is(counter.n, 4, 'initial + 3 retries, then gives up');
});
